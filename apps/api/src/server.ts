...
app.post('/api/auth/login', async (req,res) => {
  ...
  if(!user.email_verified && !['ADMIN','OWNER'].includes(user.role)) return res.status(403).json({error:'Verifica primeiro o teu email. Podes pedir um novo link de verificação.',code:'EMAIL_NOT_VERIFIED'});
...
app.get('/api/releases/:id/payment', auth, artistOnly, async (req:AuthedRequest,res) => {
  ...
  const selectedCodes=(payment?.metadata?.platformCodes || (payment?.metadata?.distributionMode==='ALL'?ALL_PLATFORM_CODES:payment?.metadata?.distributionMode==='MAIN'?MAIN_PLATFORM_CODES:[])).map((x:string)=>String(x).toUpperCase());
  const mode=payment?.metadata?.distributionMode || 'CUSTOM';
  const fullExtra=mode==='ALL'?Math.round(Number(product.amount)*0.5*100)/100:0;
  const total=mode==='ALL'?Number(product.amount)+fullExtra:Number(product.amount);
  const platformRows=(await pool.query(`select id,code,name,active,requires_isrc,requires_upc,status from distribution_platforms where active=true order by case when code='BABULO_PLAY' then 0 else 1 end,name`)).rows;
  ...
  res.json({release,product,payment,paid:Boolean(payment?.status==='PAID'),wallet:{balance:wallet,applied:walletApplied,externalDue},distribution:{mode,selectedCodes,platforms:platformRows,mainPlatformCodes:MAIN_PLATFORM_CODES,allPlatformCodes:ALL_PLATFORM_CODES,baseAmount:Number(product.amount),extraAllPlatforms:fullExtra,totalAmount:total,surchargePercent:mode==='ALL'?50:0}});
});

app.post('/api/releases/:id/payment', auth, artistOnly, async (req:AuthedRequest,res) => {
  const {paymentMethod='WALLET',fallbackPaymentMethod='MULTICAIXA_EXPRESS',distributionMode='CUSTOM',platformCodes=[]}=req.body||{};
  ...
  const explicitPlatforms=Array.isArray(platformCodes)&&platformCodes.length>0;
  let effectiveDistributionMode=String(distributionMode);
  let selectedCodes:string[]=explicitPlatforms?[...new Set(platformCodes.map((x:any)=>String(x).trim().toUpperCase()).filter(Boolean))]:platformPlanCodes(effectiveDistributionMode);
  if(!explicitPlatforms && !['MAIN','ALL'].includes(effectiveDistributionMode)) return res.status(400).json({error:'Seleciona pelo menos uma plataforma para o lançamento.'});
  if(explicitPlatforms) effectiveDistributionMode='CUSTOM';
  if(!selectedCodes.length) return res.status(400).json({error:'Seleciona pelo menos uma plataforma para o lançamento.'});
  const platformRows=(await pool.query(`select id,code,name,active from distribution_platforms where code=any($1::text[]) and active=true`,[selectedCodes])).rows;
  if(platformRows.length!==selectedCodes.length) return res.status(400).json({error:'Uma ou mais plataformas selecionadas não estão disponíveis.'});
  ...
  if(existingPaid && (existingMode==='CUSTOM'||effectiveDistributionMode==='CUSTOM')) return res.json({payment:existingPaid,paid:true,message:'Pagamento deste lançamento já confirmado. Os destinos selecionados não podem ser alterados após o pagamento.'});
  ...
  if(existingPaid && existingMode==='MAIN' && effectiveDistributionMode==='ALL'){ amount=Math.round(Number(product.amount)*0.5*100)/100; paymentType='PLATFORM_UPGRADE'; }
  else if(existingPaid && effectiveDistributionMode==='MAIN') return res.json({payment:existingPaid,paid:true,message:'Pagamento base já confirmado.'});
  ...
  try{
    ...
    await client.query('commit');
    await audit(req.user!.id,'RELEASE_PAYMENT_CREATED', 'PAYMENT',q.rows[0].id,{releaseId:release.id,productCode,paymentMethod:effectiveMethod,distributionMode:effectiveDistributionMode,platformCodes:selectedCodes,paymentType,amount,walletAmount,externalAmount});
...
app.post('/api/admin/releases/:id/decision', auth, requireAdminPermission('RELEASES'), async (req:AuthedRequest,res) => {
  ...
  try { await client.query('begin');
    ...
    if(decision==='APPROVED'){
      try{
        const paid=(await pool.query(`select metadata from payment_transactions where release_id=$1 and service_type='DISTRIBUTION' and status='PAID' order by paid_at desc limit 1`,[r.id])).rows[0];
        const selected=[...new Set(((paid?.metadata?.platformCodes||[]).map((x:any)=>String(x).toUpperCase())).filter(Boolean))];
        if(selected.length){
          const exists=(await pool.query(`select id from distribution_orders where release_id=$1 order by created_at desc limit 1`,[r.id])).rows[0];
          if(!exists){
            const rel=(await pool.query(`select r.id,r.type,a.id artist_id from releases r join artists a on a.id=r.primary_artist_id where r.id=$1`,[r.id])).rows[0];
            const prodCode=RELEASE_PAYMENT_PRODUCT[String(rel.type)];
            const prod=(await pool.query(`select code,amount,currency from payment_products where code=$1 and service_type='DISTRIBUTION' and active=true`,[prodCode])).rows[0];
            const plats=(await pool.query(`select id,code,name,requires_isrc from distribution_platforms where code=any($1::text[]) and active=true`,[selected])).rows;
            if(rel&&prod&&plats.length===selected.length){
              const od=(await pool.query(`insert into distribution_orders(release_id,artist_id,product_code,amount,currency,status) values($1,$2,$3,$4,$5,'READY') returning id`,[r.id,rel.artist_id,prod.code,prod.amount,prod.currency])).rows[0];
              let mappingRequired=false;
              for(const p of plats){
                let dStatus='READY';
                if(p.code==='BABULO_PLAY'){
                  dStatus='PUBLISHED';
                }else{
                  const profile=(await pool.query(`select profile_mode,mapping_status from artist_platform_profiles where artist_id=$1 and platform_id=$2 limit 1`,[rel.artist_id,p.id])).rows[0];
                  const mapping=profile?.profile_mode==='EXISTING'?(profile.mapping_status||'PENDING'):'NOT_REQUIRED';
                  if(profile?.profile_mode==='EXISTING'&&mapping!=='VERIFIED'){dStatus='MAPPING_REQUIRED';mappingRequired=true;}
                }
                const d=(await pool.query(`insert into distribution_deliveries(order_id,platform_id,status,delivered_at,published_at) values($1,$2,$3,case when $3='PUBLISHED' then now() else null end,case when $3='PUBLISHED' then now() else null end) returning id`,[od.id,p.id,dStatus])).rows[0];
                await pool.query(`insert into distribution_history(order_id,delivery_id,old_status,new_status,message,actor_user_id) values($1,$2,$3,$4,$5,$6)`,[od.id,d.id,'READY',dStatus,p.code==='BABULO_PLAY'?'Lançamento publicado na BaBuLo Play':'Destino preparado para distribuição',req.user!.id]);
              }
              await pool.query(`update distribution_orders set status=$1,updated_at=now() where id=$2`,[mappingRequired?'MAPPING_REQUIRED':'READY',od.id]);
              if(selected.includes('BABULO_PLAY')){
                await pool.query(`update tracks set status='PUBLISHED',updated_at=now() where primary_release_id=$1`,[r.id]);
                await pool.query(`update releases set status='PUBLISHED',updated_at=now() where id=$1`,[r.id]);
              }
            }
          }
        }
      }catch(distErr){console.error('distribution order creation after approval failed',distErr);}
    }
