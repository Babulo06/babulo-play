...
function AdminPersonalWorkspace({token,me,onLogout}:{token:string;me:any;onLogout:()=>void}){
 const labels:any={USERS:'Ouvintes / contas',ARTISTS:'Artistas',TEAM:'Equipa administrativa',RELEASES:'Lançamentos',FINANCE:'Finanças',WITHDRAWALS:'Levantamentos',MODERATION:'Moderação',ANALYTICS:'Audiência / analytics',SETTINGS:'Configurações'};
 const [profile,setProfile]=useState<any>(me?.user||{}),[fullName,setFullName]=useState(me?.user?.full_name||''),[phone,setPhone]=useState(me?.user?.phone||''),[birthDate,setBirthDate]=useState(me?.user?.birth_date?String(me.user.birth_date).slice(0,10):''),[msg,setMsg]=useState(''),[busy,setBusy]=useState(false);
 async function save(){setBusy(true);setMsg('');try{const r=await fetch(API+'/api/admin/me/profile',{method:'PATCH',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({fullName,phone,birthDate})});const d=await r.json();if(!r.ok)throw Error(d.error||'Não foi possível guardar o perfil');setProfile(d.user);setMsg('✓ Perfil atualizado.')}catch(e:any){setMsg(e.message)}finally{setBusy(false)}}
 ...
 return <WorkspaceShell title="Área do ADMIN" subtitle="O teu perfil e as funções autorizadas pelo Owner." onLogout={onLogout}>
  <section className="workspacePanel"><div className="panelTitle"><div><small>MEU PERFIL</small><h2>{profile.full_name||'Administrador'}</h2><p>{profile.admin_title||'Administrador'} · {profile.status}</p></div></div>
   <div className="metricGrid"><div className="metricCard"><small>NOME COMPLETO</small><strong>{profile.full_name||'—'}</strong><span>Dados pessoais</span></div><div className="metricCard"><small>TELEFONE</small><strong>{profile.phone||'—'}</strong><span>{profile.phone_verified?'✓ Telefone verificado':'Telefone não verificado'}</span></div><div className="metricCard"><small>IDADE</small><strong>{age===null?'—':`${age} anos`}</strong><span>{profile.birth_date?`Nascimento: ${new Date(profile.birth_date).toLocaleDateString('pt-AO')}`:'Data não definida'}</span></div><div className="metricCard"><small>ESTADO</small><strong>{profile.status||'—'}</strong><span>{profile.email_verified?'✓ Email verificado':'Verificação de email pendente'}</span></div></div>
   <div className="releaseList"><h3>Editar os meus dados</h3><label>Nome completo<input value={fullName} onChange={e=>setFullName(e.target.value)} /></label><label>Telefone<input value={phone} onChange={e=>setPhone(e.target.value)} /></label><label>Data de nascimento<input type="date" value={birthDate} onChange={e=>setBirthDate(e.target.value)} /></label><button className="primary" disabled={busy} onClick={save}>{busy?'A guardar…':'Guardar alterações'}</button>{msg&&<div className={msg.startsWith('✓')?'successMsg':'formError'}>{msg}</div>}</div>
  </section>
  <section className="workspacePanel"><div className="panelTitle"><div><small>PERMISSÕES DO OWNER</small><h2>{profile.admin_title||'Administrador'}</h2><p>Estas são as únicas funções autorizadas para a tua conta.</p></div></div><div className="metricGrid">{(Array.isArray(profile.admin_permissions)?profile.admin_permissions:[]).map((p:string)=><div className="metricCard" key={p}><small>✓ AUTORIZADO</small><strong>{labels[p]||p}</strong><span>Permissão atribuída pelo Owner</span></div>)}</div>{!(profile.admin_permissions||[]).length&&<p className="emptyState">Nenhuma função atribuída.</p>}</section>
  <section className="workspacePanel"><div className="panelTitle"><div><small>SEGURANÇA</small><h2>Proteção da conta</h2></div><strong>{profile.mfa_enabled?'🟢 2FA ATIVO':'🟠 2FA NÃO CONFIGURADO'}</strong></div><p>O 2FA protege a tua conta administrativa.</p><Security2FA token={token} me={{user:profile}}/></section>
 </WorkspaceShell>
}

function OwnerWorkspace({token,me,onLogout}:{token:string;me:any;onLogout:()=>void}){
 const [section,setSection]=useState<'home'|'accounts'|'releases'|'finance'|'analytics'|'security'>('home');
 ...
 const nav=[['home','⌂','Painel Principal'],['accounts','♙','Gestão de Contas'],['releases','♫','Lançamentos'],['finance','◉','Finanças'],['analytics','▣','Audiência'],['security','◆','Segurança']] as const;
 ...
 return <main className="ownerConsole">
   ...
   <section className="ownerMain"><header className="ownerTop"><div className="ownerPageTitle"><span>👑</span><h1>{section==='accounts'?'Gestão de Contas':section==='releases'?'Lançamentos':section==='finance'?'Finanças':section==='analytics'?'Audiência / Analytics':section==='security'?'Segurança':'Painel Principal'}</h1></div><div className="ownerIdentity"><span>👑</span><div><b>OWNER</b><small>Acesso total</small></div><span className="ownerChevron">⌄</span></div></header>
   {msg&&<div className="formError">{msg}</div>}
   {section==='home'&&<>
...
function OwnerFinanceApprovals({token,finance,money,onChanged}:{token:string;finance:any;money:(v:number)=>string;onChanged:()=>void}){
 const [data,setData]=useState<any>({payments:[],withdrawals:[],summary:{}}),[loading,setLoading]=useState(true),[busy,setBusy]=useState<string|null>(null),[msg,setMsg]=useState(''),[tab,setTab]=useState<'payments'|'withdrawals'>('payments');
 const load=async()=>{setLoading(true);setMsg('');try{const r=await fetch(API+'/api/owner/finance/approvals',{headers:{Authorization:`Bearer ${token}`}});const d=await r.json();if(!r.ok)throw Error(d.error||'Não foi possível carregar as aprovações financeiras');setData(d)}catch(e:any){setMsg(e.message)}finally{setLoading(false)}};
 useEffect(()=>{load()},[token]);
 const decidePayment=async(id:string,decision:'APPROVED'|'REJECTED')=>{let reason='';if(decision==='REJECTED'){reason=window.prompt('Motivo da rejeição do pagamento:')||'';if(!reason.trim())return;}setBusy(id);setMsg('');try{const r=await fetch(API+'/api/owner/payments/'+id+'/decision',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({decision,reason})});const d=await r.json();if(!r.ok)throw Error(d.error||'Não foi possível guardar a decisão');setMsg(decision==='APPROVED'?'✓ Pagamento aprovado pelo Owner.':'✓ Pagamento rejeitado pelo Owner.');await load();onChanged()}catch(e:any){setMsg(e.message)}finally{setBusy(null)}};
 const decideWithdrawal=async(id:string,decision:'APPROVED'|'REJECTED')=>{let reason='';if(decision==='REJECTED'){reason=window.prompt('Motivo da rejeição do levantamento:')||'';if(!reason.trim())return;}setBusy(id);setMsg('');try{const r=await fetch(API+'/api/admin/withdrawals/'+id+'/decision',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({decision,reason})});const d=await r.json();if(!r.ok)throw Error(d.error||'Não foi possível guardar a decisão');setMsg(decision==='APPROVED'?'✓ Levantamento aprovado pelo Owner.':'✓ Levantamento rejeitado e saldo devolvido.');await load();onChanged()}catch(e:any){setMsg(e.message)}finally{setBusy(null)}};
 const sum=data.summary||{};
 return <>
  <section className="metricGrid ownerMetricGrid ownerFinanceMetrics"><div className="metricCard"><small>💳 PAGAMENTOS A APROVAR</small><strong>{Number(sum.pending_payment_count||0).toLocaleString('pt-AO')}</strong><span>{money(sum.pending_payment_amount)} pendentes</span></div><div className="metricCard"><small>🏦 LEVANTAMENTOS A APROVAR</small><strong>{Number(sum.pending_withdrawal_count||0).toLocaleString('pt-AO')}</strong><span>{money(sum.pending_withdrawal_amount)} pendentes</span></div><div className="metricCard"><small>💰 RECEITA BABULO</small><strong>{money(finance?.balances?.babuloRevenue)}</strong><span>Receita externa líquida</span></div><div className="metricCard"><small>📈 RECEITA LÍQUIDA</small><strong>{money(finance?.balances?.netAfterArtistPayments)}</strong><span>Após pagamentos aos artistas</span></div></section>
  <section className="workspacePanel ownerApprovalPanel"><div className="panelTitle"><div><small>CONTROLO FINANCEIRO</small><h2>Aprovações financeiras</h2><p>Somente o OWNER pode confirmar ou rejeitar pagamentos e levantamentos.</p></div><button className="outline" onClick={load} disabled={loading}>↻ Atualizar</button></div>
   <div className="ownerApprovalTabs"><button className={tab==='payments'?'primary':'outline'} onClick={()=>setTab('payments')}>💳 Pagamentos ({data.payments?.length||0})</button><button className={tab==='withdrawals'?'primary':'outline'} onClick={()=>setTab('withdrawals')}>🏦 Levantamentos ({data.withdrawals?.length||0})</button></div>
   {msg&&<div className="successMsg">{msg}</div>}
   {loading?<p className="emptyState">A carregar aprovações...</p>:tab==='payments'?(data.payments?.length?<div className="ownerApprovalList">{data.payments.map((x:any)=><div className="ownerApprovalRow" key={x.id}><div><b>{x.release_title||'Pagamento BaBuLo Play'}</b><small>{x.stage_name||x.full_name||'Artista'} · {x.payment_method} · Ref. {x.reference||'—'}</small><small>{new Date(x.created_at).toLocaleString('pt-AO')}</small></div><div className="ownerApprovalAmount">{money(x.amount)}</div><div className="ownerApprovalActions"><button className="primary" disabled={busy===x.id} onClick={()=>decidePayment(x.id,'APPROVED')}>✓ Aprovar</button><button className="outline" disabled={busy===x.id} onClick={()=>decidePayment(x.id,'REJECTED')}>✕ Rejeitar</button></div></div>)}</div>:<p className="emptyState">Nenhum pagamento aguarda aprovação.</p>):(data.withdrawals?.length?<div className="ownerApprovalList">{data.withdrawals.map((x:any)=><div className="ownerApprovalRow" key={x.id}><div><b>{x.stage_name||x.full_name||'Artista'}</b><small>{x.method} · destino: {x.destination}</small><small>{new Date(x.created_at).toLocaleString('pt-AO')}</small></div><div className="ownerApprovalAmount">{money(x.net_amount||x.amount)}</div><div className="ownerApprovalActions"><button className="primary" disabled={busy===x.id} onClick={()=>decideWithdrawal(x.id,'APPROVED')}>✓ Aprovar</button><button className="outline" disabled={busy===x.id} onClick={()=>decideWithdrawal(x.id,'REJECTED')}>✕ Rejeitar</button></div></div>)}</div>:<p className="emptyState">Nenhum levantamento aguarda aprovação.</p>)}
  </section>
 </>;
}
...
function AccountManagement({token,isOwner,me}:{token:string;isOwner:boolean;me?:any}){
 const ALL_PERMS=['USERS','ARTISTS','TEAM','RELEASES','FINANCE','WITHDRAWALS','MODERATION','ANALYTICS','SETTINGS'];
 const PERM_LABEL:any={USERS:'Ouvintes / contas',ARTISTS:'Artistas',TEAM:'Equipa administrativa',RELEASES:'Lançamentos',FINANCE:'Finanças',WITHDRAWALS:'Levantamentos',MODERATION:'Moderação',ANALYTICS:'Audiência / analytics',SETTINGS:'Configurações'};
 ...
 return <>
  <section className="workspacePanel"><div className="panelTitle"><div><small>GESTÃO DE CONTAS</small><h2>Gestão de contas</h2></div>{isOwner&&<button className="primary" onClick={()=>{resetAdminForm();setAdminOpen(v=>!v)}}>＋ Criar ADMIN</button>}</div>
   ...
   {msg&&<div className="formError">{msg}</div>}
   {loading?<p className="emptyState">A carregar…</p>:users.length?<div className="rankList">{users.map(u=><div className="rankRow" key={u.id} style={{cursor:'pointer'}} onClick={()=>open(u)}><span><b>{u.stage_name||u.full_name||'Utilizador'}</b><small>{tab==='artists'?'Artista':tab==='listeners'?'Ouvinte':(u.admin_title||'Administrador')} · {u.status}</small></span><span><small>{tab==='artists'?`${u.release_count||0} lançamentos · ${Number(u.valid_streams||0).toLocaleString('pt-AO')} streams`:tab==='admins'?`${(u.admin_permissions||[]).length} permissões`:''}</small><strong>{u.status}</strong></span></div>)}</div>:<p className="emptyState">Nenhuma conta encontrada.</p>}
  </section>
  {detail&&<section className="workspacePanel"><div className="panelTitle"><div><small>PERFIL COMPLETO</small><h2>{detail.artist?.stage_name||detail.user.full_name||'Perfil da conta'}</h2><p>{detail.user.role}{detail.user.admin_title?` · ${detail.user.admin_title}`:''} · {detail.user.status}</p></div><button className="outline" onClick={()=>setDetail(null)}>Fechar perfil</button></div>
   <div className="metricGrid"><div className="metricCard"><small>NOME COMPLETO</small><strong>{detail.user.full_name||'—'}</strong><span>Dados pessoais</span></div><div className="metricCard"><small>TELEFONE</small><strong>{detail.user.phone||'—'}</strong><span>{detail.user.phone_verified?'✓ Verificado':'Não verificado'}</span></div><div className="metricCard"><small>IDADE</small><strong>{age(detail.user.birth_date)===null?'—':`${age(detail.user.birth_date)} anos`}</strong><span>{detail.user.birth_date?new Date(detail.user.birth_date).toLocaleDateString('pt-AO'):'Data não definida'}</span></div><div className="metricCard"><small>ESTADO DA CONTA</small><strong>{detail.user.status}</strong><span>{detail.user.email_verified?'✓ Email verificado':'Verificação de email pendente'}</span></div></div>
   {detail.artist&&<div className="releaseList"><h3>Dados do artista</h3><div className="releaseRow"><span>Nome artístico</span><small>{detail.artist.stage_name||'—'}</small></div><div className="releaseRow"><span>Nome legal</span><small>{detail.artist.legal_name||'—'}</small></div><div className="releaseRow"><span>Biografia</span><small>{detail.artist.bio||'—'}</small></div><div className="releaseRow"><span>Localização</span><small>{detail.artist.city||'—'} · {detail.artist.country||'—'}</small></div></div>}
   {detail.user.role==='ADMIN'&&<div className="releaseList"><h3>Função e permissões</h3><div className="releaseRow"><span>Função</span><small>{detail.user.admin_title||'Administrador'}</small></div><div className="metricGrid">{(detail.user.admin_permissions||[]).map((p:string)=><div className="metricCard" key={p}><small>✓ PERMITIDO</small><strong>{PERM_LABEL[p]||p}</strong><span>Atribuído pelo Owner</span></div>)}</div>{isOwner&&<button className="primary" onClick={()=>startEditAdmin(detail.user)}>✎ Mudar função e permissões</button>}</div>}
   ...
   {detail.sessions?.length>0&&<div className="releaseList"><h3>Sessões</h3>{detail.sessions.map((x:any)=><div className="releaseRow" key={x.id}><span>{x.revoked_at?'Revogada':'Sessão'}</span><small>Criada {new Date(x.created_at).toLocaleString('pt-AO')} · última atividade {x.last_activity_at?new Date(x.last_activity_at).toLocaleString('pt-AO'):'—'}</small></div>)}</div>}
   {detail.logs?.length>0&&<div className="releaseList"><h3>Histórico de auditoria</h3>{detail.logs.slice(0,50).map((l:any)=><div className="releaseRow" key={l.id}><span>{l.action}</span><small>{l.actor_name||l.actor_role||'Sistema'} · {new Date(l.created_at).toLocaleString('pt-AO')}</small></div>)}</div>}
  </section>}
