...
function AdminWorkspace({token,me,onLogout}:{token:string;me:any;onLogout:()=>void}){
 const [finance,setFinance]=useState<any>(null),[analytics,setAnalytics]=useState<any>(null),[pending,setPending]=useState<any[]>([]),[range,setRange]=useState(30),[tab,setTab]=useState<'overview'|'analytics'|'approvals'>('overview'),[msg,setMsg]=useState('');
 ...
 return <WorkspaceShell title={me?.user?.role==='OWNER'?'Área do Owner':'Área Administrativa'} subtitle="Finanças, audiência, aprovações e controlo operacional." onLogout={onLogout}>
  <div className="workspaceActions"><button className={tab==='overview'?'primary':'outline'} onClick={()=>setTab('overview')}>Visão geral</button><button className={tab==='analytics'?'primary':'outline'} onClick={()=>setTab('analytics')}>Audiência</button><button className={tab==='approvals'?'primary':'outline'} onClick={()=>setTab('approvals')}>Aprovações</button><select value={range} onChange={e=>setRange(Number(e.target.value))}><option value={1}>Hoje</option><option value={7}>7 dias</option><option value={30}>30 dias</option><option value={90}>90 dias</option><option value={365}>12 meses</option></select></div>
  {msg&&<div className="formError">{msg}</div>}
  ...
  {tab==='approvals'&&<section className="workspacePanel"><div className="panelTitle"><div><small>OPERAÇÃO</small><h2>Lançamentos pendentes</h2></div></div>{pending.length?pending.map((r:any)=><div className="workspaceRelease" key={r.id}><span><b>{r.title}</b><small>{r.type} · {r.status}</small></span><strong>{r.status}</strong></div>):<p className="emptyState">Nenhum lançamento pendente.</p>}</section>}
 </WorkspaceShell>
}
