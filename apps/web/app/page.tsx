...
function AccountManagement({token,isOwner,me}:{token:string;isOwner:boolean;me?:any}){
 ...
 const [adminOpen,setAdminOpen]=useState(false),[adminEmail,setAdminEmail]=useState(''),[adminPassword,setAdminPassword]=useState(''),[adminTitle,setAdminTitle]=useState(''),[adminPerms,setAdminPerms]=useState<string[]>(['USERS']),[adminPasswordVisible,setAdminPasswordVisible]=useState(false);
 ...
 async function createAdmin(){try{const r=await fetch(API+'/api/owner/admins',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({email:adminEmail,password:adminPassword,adminTitle,permissions:adminPerms})});const d=await r.json();if(!r.ok)throw Error(d.error||'Não foi possível criar o ADMIN');setMsg('✓ Conta ADMIN criada com a função e permissões definidas.');setAdminEmail('');setAdminPassword('');setAdminTitle('');setAdminPerms(['USERS']);setAdminOpen(false);setTab('admins');await load()}catch(e:any){setMsg(e.message)}}
 const tabAllowed=(p:string)=>can(p);
 return <>
  <section className="workspacePanel"><div className="panelTitle"><div><small>GESTÃO DE CONTAS</small><h2>Perfis separados</h2><p>Cada aba mostra apenas o tipo de conta selecionado. Clique num perfil para abrir toda a informação e executar as ações administrativas.</p></div>{isOwner&&<button className="primary" onClick={()=>setAdminOpen(v=>!v)}>＋ Criar ADMIN</button>}</div>
