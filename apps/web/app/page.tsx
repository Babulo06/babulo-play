...
function ReleaseWizard({token,onClose,onDone}:{token:string;onClose:()=>void;onDone:()=>void}){
 ...
 const [payment,setPayment]=useState<any>(null),[paymentProduct,setPaymentProduct]=useState<any>(null),[paymentPaid,setPaymentPaid]=useState(false),[paymentMethod,setPaymentMethod]=useState('MULTICAIXA_EXPRESS');
 ...
 async function loadPayment(rid:string){const r=await fetch(API+`/api/releases/${rid}/payment`,{headers:{Authorization:`Bearer ${token}`}});const d=await r.json();if(!r.ok)throw Error(d.error||'Não foi possível carregar o pagamento');setPayment(d.payment);setPaymentProduct(d.product);setPaymentPaid(Boolean(d.paid));}
 ...
 async function finish(){setBusy(true);setMsg('');try{
   ...
   for(let i=0;i<tracks.length;i++){
     const t=tracks[i];
     const tr=await fetch(API+'/api/tracks',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({title:t.title,releaseId:rid,trackNumber:i+1,genreId:t.genreId||form.genreId,language:t.language,version:t.version,durationMs:null,isExplicit:t.isExplicit,isrc:t.isrc,originalReleaseDate:form.releaseDate,fileUrl:t.audioUrl,fileType:'AUDIO',composer:t.composer,lyricist:t.lyricist,producer:t.producer,performer:t.performer,publisher:t.publisher,explicitReason:t.explicitReason,featuredArtists:t.featuredArtists,audioType:t.audioType,aiGenerated:t.aiGenerated,aiUsage:t.aiUsage,lyrics:t.lyrics})});
   ...
   setPreflight(null);setMsg(`Rascunho criado com ${tracks.length} ${tracks.length===1?'faixa':'faixas'} e declarações de direitos. Executa o preflight.`);setStep(4);
 ...
 <div className="trackAudio wide"><div><small>ÁUDIO DA FAIXA {i+1}</small><b>{t.audioName||'Ainda não enviado'}</b><p>O áudio pertence exclusivamente a esta faixa.</p></div><UploadBox title="Adicionar áudio" accept="audio/wav,audio/x-wav,audio/flac,audio/mpeg,.wav,.flac,.mp3" name={t.audioName} onChange={async f=>{try{setBusy(true);const url=await upload(f,'audio');updateTrack(i,'audioUrl',url);updateTrack(i,'audioName',f.name);setMsg(`Áudio da Faixa ${i+1} enviado.`);}catch(e:any){setMsg(e.message)}finally{setBusy(false)}}}/></div></div></div>)}
 ...
 {step===3&&<div className="uploadGrid"><UploadBox title="Capa" accept="image/jpeg,image/png" name={form.coverName} onChange={async f=>{try{setBusy(true);const url=await upload(f,'cover');set('coverUrl',url);set('coverName',f.name);setMsg('Capa enviada.');}catch(e:any){setMsg(e.message)}finally{setBusy(false)}}}/><div className="rules"><h3>Ficheiros do lançamento</h3><p>✓ Cada faixa já tem o seu próprio áudio na etapa <b>Faixas</b>.</p><p>✓ Capa: JPG/PNG, quadrada, recomendação 3000×3000.</p><p>✓ Áudio: WAV/FLAC preferencial; MP3 aceite.</p><p>✓ Evitar QR codes, URLs, preços e logótipos não autorizados na capa.</p><p>✓ O lançamento começa como <b>DRAFT</b> até passar pela análise técnica e de direitos.</p></div></div>}
 ...
 {step===5&&<div className="paymentPanel"><div className="paymentHero"><small>PAGAMENTO DO LANÇAMENTO</small><h3>{form.title||'Novo lançamento'}</h3><p>O pagamento pertence somente a este {form.type==='SINGLE'?'single':form.type==='EP'?'EP':'álbum'}.</p></div>{paymentProduct&&<div className="paymentAmount"><span>{paymentProduct.name}</span><strong>{Number(paymentProduct.amount).toLocaleString('pt-AO')} {paymentProduct.currency}</strong></div>}<div className="paymentStatus"><b>{paymentPaid?'✓ Pagamento confirmado':'⏳ Pagamento pendente'}</b><p>{paymentPaid?'O pagamento foi confirmado. Depois de concluir o preflight, podes lançar o projeto.':'Podes pagar agora ou continuar a preencher o lançamento. Sem pagamento confirmado, o lançamento permanece em rascunho.'}</p></div>{!paymentPaid&&<><label>Método de pagamento<select value={paymentMethod} onChange={e=>setPaymentMethod(e.target.value)}><option value="MULTICAIXA_EXPRESS">Multicaixa Express</option><option value="MULTICAIXA_REFERENCE">Multicaixa — Referência</option><option value="VISA">Visa</option><option value="MASTERCARD">Mastercard</option></select></label><button type="button" className="primary" disabled={busy} onClick={createPayment}>{busy?'A processar…':'💳 Pagar lançamento'}</button></>}{payment&&<div className="paymentReceipt"><p><b>Estado:</b> {payment.status}</p><p><b>Referência:</b> {payment.reference}</p><p><b>Valor:</b> {Number(payment.amount).toLocaleString('pt-AO')} {payment.currency}</p><small>Depois de efetuares o pagamento, aguarda a confirmação da BaBuLo Play e volta aqui para atualizar o estado.</small></div>}<button type="button" className="outline" onClick={async()=>{if(releaseId){try{setBusy(true);await loadPayment(releaseId)}catch(e:any){setMsg(e.message)}finally{setBusy(false)}}}} disabled={busy}>↻ Atualizar estado do pagamento</button></div>}
