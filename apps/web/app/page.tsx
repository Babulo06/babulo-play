...
function OwnerWorkspace({token,me,onLogout}:{token:string;me:any;onLogout:()=>void}){
 ...
 return <main className="ownerConsole">
   ...
   {section==='finance'&&<><section className="metricGrid ownerMetricGrid"><div className="metricCard"><small>🎵 SALDO DE STREAMS</small><strong>{Number(b.streamBalance?.validStreams||0).toLocaleString('pt-AO')}</strong><span>Streams válidos</span></div><div className="metricCard"><small>💳 PAGAMENTOS ARTISTAS</small><strong>{money(b.artistPayments?.paid)}</strong><span>Concluídos</span></div><div className="metricCard"><small>💰 ROYALTIES</small><strong>{money(b.artistRoyalties)}</strong><span>A pagar aos artistas</span></div><div className="metricCard"><small>📊 RECEITA BABULO</small><strong>{money(b.babuloRevenue)}</strong><span>Receita externa líquida</span></div><div className="metricCard"><small>🏦 LEVANTAMENTOS</small><strong>{money(b.artistPayments?.pending)}</strong><span>Pendentes</span></div><div className="metricCard"><small>↩️ ESTORNOS</small><strong>{money(b.refunds)}</strong><span>Transações estornadas</span></div><div className="metricCard"><small>📈 RECEITA LÍQUIDA</small><strong>{money(b.netAfterArtistPayments)}</strong><span>Após pagamentos</span></div><div className="metricCard"><small>⏳ PAGAMENTOS PENDENTES</small><strong>{money(b.pendingPayments)}</strong><span>Aguardando confirmação</span></div></section></>}
