...
function AccountManagement({token,isOwner,me}:{token:string;isOwner:boolean;me?:any}){
 ...
 return <>
  ...
  {editingAdmin&&isOwner&&<div className="modalBackdrop"><div className="authModal" style={{maxWidth:720}}><button className="modalClose" onClick={()=>setEditingAdmin(null)}>✕</button><h2>Editar ADMIN</h2><p>Altera a função e as permissões deste administrador.</p><label>Função<select value={roleTemplate} onChange={e=>applyTemplate(e.target.value)}>{Object.keys(ROLE_TEMPLATES).map(x=><option key={x}>{x}</option>)}</select></label><label>Nome da função / cargo<input value={adminTitle} onChange={e=>setAdminTitle(e.target.value)} /></label><label>Nome completo<input value={adminFullName} onChange={e=>setAdminFullName(e.target.value)} /></label><label>Telefone<input value={adminPhone} onChange={e=>setAdminPhone(e.target.value)} /></label><label>Data de nascimento<input type="date" value={adminBirthDate} onChange={e=>setAdminBirthDate(e.target.value)} /></label><div><h4>Permissões</h4>{ALL_PERMS.map(p=><label className="check" key={p}><input type="checkbox" checked={adminPerms.includes(p)} onChange={()=>togglePerm(p)}/>{PERM_LABEL[p]}</label>)}</div><button className="primary full" disabled={!adminTitle.trim()||!adminPerms.length} onClick={saveAdminEdit}>Guardar função e permissões</button></div></div>}
 </>
}
