...
function AdminPersonalWorkspace({token,me,onLogout}:{token:string;me:any;onLogout:()=>void}){
 const perms:string[]=Array.isArray(me?.user?.admin_permissions)?me.user.admin_permissions:[];
 const labels:any={USERS:'Gestão de ouvintes / contas',ARTISTS:'Gestão de artistas',TEAM:'Gestão de admins / equipa',RELEASES:'Lançamentos e aprovações',FINANCE:'Finanças',WITHDRAWALS:'Levantamentos',MODERATION:'Moderação',ANALYTICS:'Audiência / analytics',SETTINGS:'Configurações'};
 ...
 return <WorkspaceShell title="Área do ADMIN" subtitle="Perfil pessoal e funções atribuídas pelo Owner." onLogout={onLogout}>
  <section className="workspacePanel"><div className="panelTitle"><div><small>MEU PERFIL</small><h2>{profile.full_name||profile.email||'Administrador'}</h2><p>{profile.admin_title||'Administrador'} · {profile.status}</p></div></div>
   <div className="metricGrid"><div className="metricCard"><small>NOME COMPLETO</small><strong>{profile.full_name||'—'}</strong><span>Identificação da conta</span></div><div className="metricCard"><small>EMAIL</small><strong>{profile.email||'—'}</strong><span>{profile.email_verified?'✓ Email verificado':'Email não verificado'}</span></div><div className="metricCard"><small>TELEFONE</small><strong>{profile.phone||'—'}</strong><span>{profile.phone_verified?'✓ Telefone verificado':'Telefone não verificado'}</span></div><div className="metricCard"><small>IDADE</small><strong>{age===null?'—':`${age} anos`}</strong><span>{profile.birth_date?`Nascimento: ${new Date(profile.birth_date).toLocaleDateString('pt-AO')}`:'Data não definida'}</span></div></div>
   <div className="releaseList"><h3>Editar dados pessoais</h3><label>Nome completo<input value={fullName} onChange={e=>setFullName(e.target.value)} /></label><label>Telefone<input value={phone} onChange={e=>setPhone(e.target.value)} /></label><label>Data de nascimento<input type="date" value={birthDate} onChange={e=>setBirthDate(e.target.value)} /></label><button className="primary" disabled={busy} onClick={save}>{busy?'A guardar…':'Guardar perfil'}</button>{msg&&<div className={msg.startsWith('✓')?'successMsg':'formError'}>{msg}</div>}</div>
  </section>
  <section className="workspacePanel"><div className="panelTitle"><div><small>ACESSOS ATRIBUÍDOS PELO OWNER</small><h2>As tuas funções</h2><p>Este painel mostra apenas as permissões que o Owner atribuiu a esta conta.</p></div></div><div className="metricGrid">{perms.map(p=><div className="metricCard" key={p}><small>✓ PERMITIDO</small><strong>{labels[p]||p}</strong><span>Permissão ativa</span></div>)}</div>{!perms.length&&<p className="emptyState">Nenhuma função foi atribuída. Contacta o Owner.</p>}</section>
  <section className="workspacePanel"><div className="panelTitle"><div><small>SEGURANÇA</small><h2>Proteção da conta</h2></div><strong>{profile.mfa_enabled?'🟢 2FA ATIVO':'🟠 2FA NÃO CONFIGURADO'}</strong></div><p>A autenticação em duas etapas protege a tua conta de ADMIN.</p><Security2FA token={token} me={{user:profile}}/></section>
 </WorkspaceShell>
}
...
function AccountManagement({token,isOwner,me}:{token:string;isOwner:boolean;me?:any}){
 ...
 const PERM_LABEL:any={USERS:'Ouvintes / contas',ARTISTS:'Artistas',TEAM:'Admins / equipa',RELEASES:'Lançamentos',FINANCE:'Finanças',WITHDRAWALS:'Levantamentos',MODERATION:'Moderação',ANALYTICS:'Audiência / analytics',SETTINGS:'Configurações'};
 const myPerms:string[]=isOwner?ALL_PERMS:(Array.isArray(me?.user?.admin_permissions)?me.user.admin_permissions:ALL_PERMS);
 const can=(p:string)=>isOwner||myPerms.includes(p);
 const [tab,setTab]=useState<'artists'|'listeners'|'admins'>('artists');
 const [users,setUsers]=useState<any[]>([]),[q,setQ]=useState(''),[status,setStatus]=useState(''),[selected,setSelected]=useState<any>(null),[detail,setDetail]=useState<any>(null),[loading,setLoading]=useState(false),[msg,setMsg]=useState('');
 const [adminOpen,setAdminOpen]=useState(false),[adminEmail,setAdminEmail]=useState(''),[adminPassword,setAdminPassword]=useState(''),[adminTitle,setAdminTitle]=useState(''),[adminFullName,setAdminFullName]=useState(''),[adminPhone,setAdminPhone]=useState(''),[adminBirthDate,setAdminBirthDate]=useState(''),[adminPerms,setAdminPerms]=useState<string[]>(['USERS']),[adminPasswordVisible,setAdminPasswordVisible]=useState(false);
 const role=tab==='artists'?'ARTIST':tab==='listeners'?'LISTENER':'ADMIN';
 useEffect(()=>{if(!can(tab==='artists'?'ARTISTS':tab==='listeners'?'USERS':'TEAM'))return;load()},[tab,status]);
 async function load(){setLoading(true);setMsg('');try{const qs=new URLSearchParams();qs.set('role',role);if(q.trim())qs.set('q',q.trim());if(status)qs.set('status',status);const r=await fetch(API+'/api/admin/users?'+qs.toString(),{headers:{Authorization:`Bearer ${token}`}});const d=await r.json();if(!r.ok)throw Error(d.error||'Não foi possível carregar as contas');setUsers(d.users||[])}catch(e:any){setMsg(e.message)}finally{setLoading(false)}}
 async function open(u:any){setSelected(u);setDetail(null);setMsg('');try{const r=await fetch(API+`/api/admin/users/${u.id}`,{headers:{Authorization:`Bearer ${token}`}});const d=await r.json();if(!r.ok)throw Error(d.error||'Não foi possível abrir o perfil');setDetail(d)}catch(e:any){setMsg(e.message)}}
 async function changeStatus(u:any,next:string){if(u.status===next)return;if(!window.confirm(`${next==='BLOCKED'?'Bloquear':next==='SUSPENDED'?'Suspender':'Ativar'} ${u.stage_name||u.email||u.phone}?`))return;try{const r=await fetch(API+`/api/admin/users/${u.id}/status`,{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({status:next})});const d=await r.json();if(!r.ok)throw Error(d.error||'Não foi possível alterar o estado');setMsg('✓ Estado atualizado.');await load();await open(d.user)}catch(e:any){setMsg(e.message)}}
 async function verify(a:any,next:string){try{const r=await fetch(API+`/api/admin/artists/${a.id}/verification`,{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({verificationStatus:next})});const d=await r.json();if(!r.ok)throw Error(d.error||'Não foi possível atualizar a verificação');setMsg('✓ Verificação atualizada.');await load();if(selected)await open(selected)}catch(e:any){setMsg(e.message)}}
 async function deleteAdmin(u:any){if(!isOwner||u.role!=='ADMIN')return;if(!window.confirm(`Apagar permanentemente a conta ADMIN ${u.email||''}? Esta ação não pode ser desfeita.`))return;try{const r=await fetch(API+`/api/owner/admins/${u.id}`,{method:'DELETE',headers:{Authorization:`Bearer ${token}`}});const d=await r.json();if(!r.ok)throw Error(d.error||'Não foi possível apagar o ADMIN');setMsg('✓ Conta ADMIN apagada.');setDetail(null);setSelected(null);await load()}catch(e:any){setMsg(e.message)}}
 function togglePerm(p:string){setAdminPerms(x=>x.includes(p)?x.filter(v=>v!==p):[...x,p])}
 async function createAdmin(){try{const r=await fetch(API+'/api/owner/admins',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({email:adminEmail,password:adminPassword,adminTitle,permissions:adminPerms,fullName:adminFullName,phone:adminPhone,birthDate:adminBirthDate})});const d=await r.json();if(!r.ok)throw Error(d.error||'Não foi possível criar o ADMIN');setMsg('✓ Conta ADMIN criada com a função e permissões definidas.');setAdminEmail('');setAdminPassword('');setAdminTitle('');setAdminFullName('');setAdminPhone('');setAdminBirthDate('');setAdminPerms(['USERS']);setAdminOpen(false);setTab('admins');await load()}catch(e:any){setMsg(e.message)}}
 const tabAllowed=(p:string)=>can(p);
 return <>
  <section className="workspacePanel"><div className="panelTitle"><div><small>GESTÃO DE CONTAS</small><h2>Gestão de contas</h2></div>{isOwner&&<button className="primary" onClick={()=>setAdminOpen(v=>!v)}>＋ Criar ADMIN</button>}</div>
   <div className="workspaceActions"><button className={tab==='artists'?'primary':'outline'} disabled={!tabAllowed('ARTISTS')} onClick={()=>setTab('artists')}>🎤 Artistas</button><button className={tab==='listeners'?'primary':'outline'} disabled={!tabAllowed('USERS')} onClick={()=>setTab('listeners')}>🎧 Ouvintes</button><button className={tab==='admins'?'primary':'outline'} disabled={!tabAllowed('TEAM')} onClick={()=>setTab('admins')}>🛡️ Admins</button></div>
   {adminOpen&&isOwner&&<div className="credits" style={{marginBottom:16}}><h3>Novo administrador</h3><label>Nome completo<input value={adminFullName} onChange={e=>setAdminFullName(e.target.value)} placeholder="Nome completo"/></label><label>Telefone<input value={adminPhone} onChange={e=>setAdminPhone(e.target.value)} placeholder="+244 ..."/></label><label>Data de nascimento<input type="date" value={adminBirthDate} onChange={e=>setAdminBirthDate(e.target.value)}/></label><label>Função / cargo<input value={adminTitle} onChange={e=>setAdminTitle(e.target.value)} placeholder="Ex.: Moderador de Conteúdo"/></label><label>Email<input value={adminEmail} onChange={e=>setAdminEmail(e.target.value)} placeholder="admin@babulo.ao"/></label><label>Palavra-passe <small>(mínimo 12 caracteres)</small><div className="passwordField"><input type={adminPasswordVisible?'text':'password'} value={adminPassword} onChange={e=>setAdminPassword(e.target.value)} /><button type="button" className="passwordEye" onClick={()=>setAdminPasswordVisible(v=>!v)}>{adminPasswordVisible?'🙈':'👁️'}</button></div></label><div><h4>Permissões desta função</h4>{ALL_PERMS.map(p=><label className="check" key={p}><input type="checkbox" checked={adminPerms.includes(p)} onChange={()=>togglePerm(p)}/>{PERM_LABEL[p]}</label>)}</div><button className="primary" disabled={!adminEmail||adminPassword.length<12||!adminTitle.trim()||!adminFullName.trim()||!adminPerms.length} onClick={createAdmin}>Criar ADMIN com esta função</button></div>}
   <div className="workspaceActions"><input value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==='Enter'&&load()} placeholder={tab==='artists'?'Pesquisar artista, email ou telefone':tab==='listeners'?'Pesquisar ouvinte, email ou telefone':'Pesquisar admin, email ou telefone'}/><select value={status} onChange={e=>setStatus(e.target.value)}><option value="">Todos os estados</option><option value="ACTIVE">Ativos</option><option value="SUSPENDED">Suspensos</option><option value="BLOCKED">Bloqueados</option></select><button className="outline" onClick={load}>Pesquisar</button></div>
   {msg&&<div className="formError">{msg}</div>}
   {loading?<p className="emptyState">A carregar {role==='ARTIST'?'artistas':role==='LISTENER'?'ouvintes':'admins'}…</p>:users.length?<div className="rankList">{users.map(u=><div className="rankRow" key={u.id} style={{cursor:'pointer'}} onClick={()=>open(u)}><span><b>{u.stage_name||u.email||u.phone||'Utilizador'}</b><small>{tab==='artists'?(u.email||u.phone||'sem contacto'):(tab==='admins'?(u.admin_title||'Administrador'):(u.email||u.phone||'sem contacto'))} · {u.status}</small></span><span><small>{tab==='artists'?`${u.release_count||0} lançamentos · ${Number(u.valid_streams||0).toLocaleString('pt-AO')} streams`:''}</small><strong>{u.status}</strong></span></div>)}</div>:<p className="emptyState">Nenhuma conta encontrada nesta aba.</p>}
  </section>
  {detail&&selected&&<section className="workspacePanel"><div className="panelTitle"><div><small>PERFIL COMPLETO</small><h2>{detail.artist?.stage_name||detail.user.email||detail.user.phone||'Utilizador'}</h2><p>{detail.user.role}{detail.user.admin_title?` · ${detail.user.admin_title}`:''} · conta {detail.user.status}</p></div><button className="outline" onClick={()=>{setSelected(null);setDetail(null)}}>Fechar perfil</button></div>
   <div className="releaseList"><h3>Dados pessoais</h3><div className="releaseRow"><span>Nome completo</span><small>{detail.user.full_name||'—'}</small></div><div className="releaseRow"><span>Telefone</span><small>{detail.user.phone||'—'}</small></div><div className="releaseRow"><span>Idade</span><small>{detail.user.birth_date?Math.max(0,new Date().getFullYear()-new Date(detail.user.birth_date).getFullYear()-((new Date().getMonth()+1<new Date(detail.user.birth_date).getMonth()+1)||(new Date().getMonth()+1===new Date(detail.user.birth_date).getMonth()+1&&new Date().getDate()<new Date(detail.user.birth_date).getDate())?1:0))+' anos':'—'}</small></div></div><div className="metricGrid"><div className="metricCard"><small>ESTADO DA CONTA</small><strong>{detail.user.status}</strong><span>{detail.user.email||detail.user.phone||'Sem contacto'}</span></div><div className="metricCard"><small>CRIADA EM</small><strong>{new Date(detail.user.created_at).toLocaleDateString('pt-AO')}</strong><span>Email {detail.user.email_verified?'verificado':'não verificado'} · telefone {detail.user.phone_verified?'verificado':'não verificado'}</span></div><div className="metricCard"><small>STREAMS VÁLIDOS</small><strong>{Number(detail.streamStats?.valid_streams||0).toLocaleString('pt-AO')}</strong><span>{Number(detail.streamStats?.unique_listeners||0).toLocaleString('pt-AO')} ouvintes únicos</span></div>{detail.artist&&<><div className="metricCard"><small>VERIFICAÇÃO DO ARTISTA</small><strong>{detail.artist.verification_status}</strong><span>{detail.artist.country||'País não definido'} · {detail.artist.city||'Cidade não definida'}</span></div><div className="metricCard"><small>LANÇAMENTOS</small><strong>{detail.releases?.length||0}</strong><span>Histórico recente</span></div><div className="metricCard"><small>LEVANTAMENTOS</small><strong>{detail.withdrawals?.length||0}</strong><span>Histórico recente</span></div></>}</div>
   ...
   {detail.user.role==='ADMIN'&&<div className="releaseList"><h3>Função e permissões</h3><div className="releaseRow"><span>{detail.user.admin_title||'Administrador'}</span><small>{(detail.user.admin_permissions||[]).map((p:string)=>PERM_LABEL[p]||p).join(' · ')||'Sem permissões'}</small></div></div>}
   <div className="workspaceActions"><button className="outline" disabled={!can('USERS')&&!can('ARTISTS')&&!can('TEAM')} onClick={()=>changeStatus(detail.user,'ACTIVE')}>🟢 Ativar</button><button className="outline" disabled={!can('USERS')&&!can('ARTISTS')&&!can('TEAM')} onClick={()=>changeStatus(detail.user,'SUSPENDED')}>🟠 Desativar / Suspender</button><button className="outline" disabled={!can('USERS')&&!can('ARTISTS')&&!can('TEAM')} onClick={()=>changeStatus(detail.user,'BLOCKED')}>🔴 Bloquear</button>{detail.user.role==='ADMIN'&&isOwner&&<button className="outline" onClick={()=>deleteAdmin(detail.user)}>🗑️ Apagar ADMIN</button>}{detail.artist&&can('ARTISTS')&&<><button className="outline" onClick={()=>verify(detail.artist,'VERIFIED')}>✓ Verificar artista</button><button className="outline" onClick={()=>verify(detail.artist,'REJECTED')}>✕ Recusar verificação</button></>}</div>
   ...
   {detail.withdrawals?.length>0&&<div className="releaseList"><h3>Levantamentos</h3>{detail.withdrawals.slice(0,50).map((w:any)=><div className="releaseRow" key={w.id}><span>{Number(w.amount||0).toLocaleString('pt-AO')} {w.currency}</span><small>{w.method} · {w.status} · {new Date(w.created_at).toLocaleString('pt-AO')}</small></div>)}</div>}
   ...
   {detail.logs?.length>0&&<div className="releaseList"><h3>Histórico de auditoria</h3>{detail.logs.slice(0,50).map((l:any)=><div className="releaseRow" key={l.id}><span>{l.action}</span><small>{l.actor_email||'Sistema'} · {new Date(l.created_at).toLocaleString('pt-AO')}</small></div>)}</div>}
  </section>}
 </>
}
