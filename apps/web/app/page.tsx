...
type WizardTrack={
 ...
 rightsTerritory:string; audioUrl:string; audioName:string; durationMs:number; promoStartMs:number; promoEndMs:number;
}
...
function ReleaseWizard({token,onClose,onDone}:{token:string;onClose:()=>void;onDone:()=>void}){
 ...
 <div className="trackAudio wide"><div><small>ÁUDIO DA FAIXA {i+1}</small><b>{t.audioName||'Ainda não enviado'}</b><p>Depois do upload, o áudio pode ser ouvido, substituído ou apagado.</p>{t.audioUrl&&<AudioTrackEditor track={t} index={i} token={token} onUpdate={updateTrack} onMessage={setMsg} onBusy={setBusy}/>}</div><div className="audioUploadActions"><UploadBox title={t.audioUrl?'Substituir áudio':'Adicionar áudio'} accept="audio/wav,audio/x-wav,audio/flac,audio/mpeg,.wav,.flac,.mp3" name={t.audioName} onChange={async f=>{try{setBusy(true);const probe=new Audio();const objectUrl=URL.createObjectURL(f);probe.src=objectUrl;await new Promise<void>(resolve=>{probe.onloadedmetadata=()=>resolve();probe.onerror=()=>resolve()});const dur=Number.isFinite(probe.duration)?Math.round(probe.duration*1000):0;URL.revokeObjectURL(objectUrl);const url=await upload(f,'audio');updateTrack(i,'audioUrl',url);updateTrack(i,'audioName',f.name);updateTrack(i,'durationMs',dur);updateTrack(i,'promoStartMs',0);updateTrack(i,'promoEndMs',Math.min(dur,60000));setMsg(`Áudio da Faixa ${i+1} enviado.`);}catch(e:any){setMsg(e.message)}finally{setBusy(false)}}}/>{t.audioUrl&&<button type="button" className="outline danger" onClick={async()=>{if(!confirm('Apagar o áudio desta faixa?'))return;try{setBusy(true);const r=await fetch(API+`/api/tracks/${t.id}/audio`,{method:'DELETE',headers:{Authorization:`Bearer ${token}`}});const d=await r.json();if(!r.ok)throw Error(d.error||'Não foi possível apagar o áudio');updateTrack(i,'audioUrl','');updateTrack(i,'audioName','');updateTrack(i,'durationMs',0);updateTrack(i,'promoStartMs',0);updateTrack(i,'promoEndMs',0);setMsg(`Áudio da Faixa ${i+1} apagado.`);}catch(e:any){setMsg(e.message)}finally{setBusy(false)}}}>🗑️ Apagar áudio</button>}</div></div></div></div>)}
 ...
 {step===3&&<div className="uploadGrid"><UploadBox title="Capa" accept="image/jpeg,image/png" name={form.coverName} onChange={async f=>{try{setBusy(true);const url=await upload(f,'cover');set('coverUrl',url);set('coverName',f.name);setMsg('Capa enviada.');}catch(e:any){setMsg(e.message)}finally{setBusy(false)}}}/><div className="rules"><h3>Ficheiros do lançamento</h3><p>✓ Cada faixa já tem o seu próprio áudio na etapa <b>Faixas</b>.</p><p>✓ Capa: JPG/PNG, exatamente 600×600 px.</p><p>✓ Áudio: WAV/FLAC preferencial; MP3 aceite.</p><p>✓ Evitar QR codes, URLs, preços e logótipos não autorizados na capa.</p><p>✓ O lançamento começa como <b>DRAFT</b> até passar pela análise técnica e de direitos.</p></div></div>}
