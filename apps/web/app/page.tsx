...
function OwnerWorkspace({token,me,onLogout}:{token:string;me:any;onLogout:()=>void}){
 ...
 const [finance,setFinance]=useState<any>(null),[analytics,setAnalytics]=useState<any>(null),[pending,setPending]=useState<any[]>([]),[range,setRange]=useState(30),[msg,setMsg]=useState('');
 ...
 useEffect(()=>{load()},[range,token]);
 const money=(v:number)=>`${Number(v||0).toLocaleString('pt-AO',{maximumFractionDigits:2})} Kz`; const b=finance?.balances||{};
 ...
 return <main className="ownerConsole">
   ...
   <section className="ownerMain"><header className="ownerTop"><div><small>BA BULO PLAY · ADMINISTRAÇÃO</small><h1>{section==='accounts'?'Gestão de contas':section==='releases'?'Lançamentos':section==='finance'?'Finanças':section==='analytics'?'Audiência / Analytics':section==='security'?'Segurança':'Painel Principal'}</h1><p>Controlo central da plataforma.</p></div><div className="ownerIdentity"><span>👑</span><div><b>OWNER</b><small>Acesso total</small></div></div></header>
   {msg&&<div className="formError">{msg}</div>}
   {section==='home'&&<><div className="ownerHero"><div><small>VISÃO GERAL</small><h2>Bem-vindo ao centro de comando</h2><p>Gerencie contas, artistas, lançamentos, finanças, audiência e segurança da BaBuLo Play.</p></div><select value={range} onChange={e=>setRange(Number(e.target.value))}><option value={1}>Hoje</option><option value={7}>7 dias</option><option value={30}>30 dias</option><option value={90}>90 dias</option><option value={365}>12 meses</option></select></div><section className="metricGrid ownerMetricGrid"><div className="metricCard"><small>🎵 STREAMS</small><strong>{Number(b.streamBalance?.validStreams||analytics?.totals?.validStreams||0).toLocaleString('pt-AO')}</strong><span>Streams válidos</span></div><div className="metricCard"><small>👥 OUVINTES</small><strong>{Number(analytics?.totals?.uniqueListeners||0).toLocaleString('pt-AO')}</strong><span>Ouvintes únicos</span></div><div className="metricCard"><small>🎤 ARTISTAS</small><strong>—</strong><span>Consulte Gestão de Contas</span></div><div className="metricCard"><small>💰 RECEITA</small><strong>{money(b.babuloRevenue)}</strong><span>Receita BaBuLo Play</span></div></section><section className="workspaceGrid"><div className="workspacePanel"><div className="panelTitle"><div><small>FINANÇAS</small><h2>Resumo financeiro</h2></div><button className="outline" onClick={()=>setSection('finance')}>Abrir</button></div><div className="financeMini"><div><span>Royalties</span><b>{money(b.artistRoyalties)}</b></div><div><span>Levantamentos</span><b>{money(b.artistPayments?.pending)}</b></div><div><span>Receita líquida</span><b>{money(b.netAfterArtistPayments)}</b></div></div></div><div className="workspacePanel"><div className="panelTitle"><div><small>OPERAÇÃO</small><h2>Lançamentos pendentes</h2></div><button className="outline" onClick={()=>setSection('releases')}>Ver</button></div><p className="emptyState">{pending.length?`${pending.length} lançamento(s) aguardam análise.`:'Nenhum lançamento pendente.'}</p></div></section></>}
